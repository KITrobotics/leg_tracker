...
//
  ...
  void pubCovarianceAsEllipse(const double meanX, const double meanY, const MatrixXd& S, const int id)
  {
...
//     PointCloud sor_filtered;
  ...
  void updateLeg(Leg& l, const Point& p, std::vector<double>& in, std::vector<double>& out)
  {
    ROS_INFO("updateLeg");
    printLegsInfo();
    in.clear();
...
//     marker.mesh_resource = "package://pr2_description/meshes/base_v0/base.dae";
  ...
  void matchLegCandidates(PointCloud cluster_centroids)
  {
    ...
    for (int i = 0; i < legs.size(); i++)
    {
      ROS_DEBUG("Observation to track data association!");
