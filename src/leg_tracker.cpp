...
//     PointCloud sor_filtered;
  ...
  void visLegs(PointCloud& cloud)
  {
    ...
    for (Leg& l : legs)
    {
      ...
      ma.markers.push_back(getMarker(l.getPos().x, l.getPos().y, id, true));
      id++;
    }
...
//     marker_array_publisher.publish(ma);
  ...
  void findSecondLeg(int fst_leg)
  {
    ...
    if (indices_of_potential_legs.size() == 1) { setPeopleId(fst_leg, indices_of_potential_legs[0]); return; }
  ...
  {
    ROS_INFO("findMatch for (%f, %f) with radius %f", searchPoint.x, searchPoint.y, max_dist_btw_legs);
